First-Order Markov Chain Using Toy Data
===========================================================

`(full code) <https://github.com/assamite/cc-mas16/blob/master/week1/toy_markov.py>`_

In this section we will show with a toy example how first-order Markov chain
can be created using Python. We start by creating the state transition
probabilities for the artificial data, and then generate text using these
probabilities.
...
draw from that distribution the target number of items, and concatenate them
to another string. ::
...
structure, in some cases it might be profitable to add some small transition
...
generate *N* characters overall. Then we loop generation loop until we have
